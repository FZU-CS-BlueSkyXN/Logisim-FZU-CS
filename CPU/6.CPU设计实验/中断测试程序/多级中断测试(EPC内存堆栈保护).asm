...
addiu $s6,$0,8  # Magic lamp Counter
zmd_loop:
...
addi $s0, $s0, 1   # next maigic lamp number
addi $t8,$0,15
and $s0, $s0, $t8
sll $s0, $s0, 28     
...
#############################################################################################
#  exceptoin 1
#  save pertinent registers  EPC? $s6? $s5?$s4?$s3?$s0?$a0?$v0
sw $v0, 0($sp)
addi $sp, $sp, 4
sw $a0, 0($sp)
addi $sp, $sp, 4
...
# sets the Interrupt Enable Bit=0 . This allows the processor to respond to maskable hardware interrupts.
mtc0 $zero, $12   # $12 is IE in CP0
...
# sets the interrupt Enable Bit=1 . This ignores external interrupts
addi $s6, $zero, 1
mtc0 $s6, $12


#  restore pertinent registers  EPC? $s6? $s5?$s4?$s3?$s0?$a0?$v0
addi $sp, $sp, -4
lw $s6, 0($sp)
mtc0 $s6, $14        # $14 is EPC Register in CP0
